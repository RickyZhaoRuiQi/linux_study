...
//进程池类，将它定义位模板类是为了代码复用，其模板参数是处理逻辑任务的类
...
//进程池构造函数，参数listenfd是监听sockfd，它必须在创建进程池之前被创建，否则子进程无法直接引用它。参数
...
//父进程中m_idx为-1,子进程中m_idx值大于等于0，我们据此判断下来要运行的是父进程代码还是子进程代码
template<typename T>
...
void processpool<T>::run_child()
{
	...
	while(!m_stop)
	{
		...
		for(int i = 0;i < number;++i)
		{
			...
			{
				int client = 0;
				//从父、子进程之间的管道读取数据，并将结果保存在变量client中。如果读取成功，则表示有新客户连接
				...
				else
				{
					...
					//模板类T必须实现init方法，以初始化一个客户连接，我们直接使用connfd来索引逻辑处理对象(T类型
					//对象)，以提高程序效率
			...
			}//如果是其他数据，那么必然是客户端请求到来。调用逻辑处理对象process方法处理
						...
						switch(signals[i])
						  {
							  case SIGCHLD:
								  {
									  ...
									  while((pid = waitpid(-1,&stat,WNOHANG)) > 0)
									  {
										  ...
										  {	//如果进程池中第i个子进程退出了，则主进程关闭相应的通信通道，并
											//设置相应的m_pid为-1，以标记该子进程已经退出
							  ...
							  case SIGINT:
								  {	//如果父进程接收到终止信号，那么就杀死所有子进程，并等待它们全部结束。
									//通知子进程结束更好的方法是向父、子进程之间的通信管道发出特殊数据
